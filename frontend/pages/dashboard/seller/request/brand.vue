					...
					<div class="bg-white p-3 min-h100 rounded card-primary">
						...
						<button type="submit" class="btn btn-primary mt-5">
							<transition name="fade" mode="out-in">
								<Spiner v-if="loading" />
								<span v-else>Request Brand</span>
							</transition>
						</button>
					</div>
				</div>
...
<script>
	export default {
		name: "new-brand-request",
		middleware: "seller",
		head() {
			return {
				title: `New Brand Request - ${this.appName}`,
			};
		},
		...
		methods: {
			...
			submit() {
				if (this.click) {
					...
					this.$axios.post("request-brand", formData, config).then(
						(response) => {
							$nuxt.$emit("success", response.data);
							this.$router.push(
								this.localePath("dashboard-seller-request")
							);
							this.click = true;
						},
						(error) => {
