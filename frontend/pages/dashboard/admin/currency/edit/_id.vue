...
<script>
	export default {
		...
		methods: {
			// Get Currency
			editCurrency() {
				this.$axios.get(`edit-currency/${this.$route.params.id}`).then(
					(response) => {
						this.form.name = response.data.currency.name;
						this.form.country = response.data.currency.country;
						this.form.symble = response.data.currency.symble;
						this.form.rate = response.data.currency.rate;
						this.form.status = response.data.currency.status;
					},
					(error) => {
			...
			submit() {
				if (this.click) {
					this.click = false;
					this.$axios
						.post(`update-currency/${this.$route.params.id}`, this.form)
						.then(
							(response) => {
